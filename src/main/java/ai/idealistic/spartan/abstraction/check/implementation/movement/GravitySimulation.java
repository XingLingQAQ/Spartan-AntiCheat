...
public class GravitySimulation extends CheckRunner {
    ...
    private int teleportTicks = 0; // vagdedes patch
    private double motionY = 0;

    public GravitySimulation(HackType hackType, PlayerProtocol protocol) {
        ...
        this.detection.call(() -> {
            ...
            if (to.getX() != from.getX() || to.getY() != from.getY() || to.getZ() != from.getZ()) {
                ...
                if (p.getComponentY().pistonTick)
                    this.pistonTicks = 2;
                if (this.getDeviance(motion) > 0.01
                ...
                boolean slimeEvent = (this.getDeviance(motion) > 0.001)
                ...
                double waterLim = (MultiVersion.isOrGreater(MultiVersion.MCVersion.V1_13))
                ...
                if (this.getDeviance(motion) > 0.001
                        && this.slimePunch > 0
                ...
                if (this.getDeviance(motion) > 0.001
                        && this.teleportTicks > 0
                        && this.teleportExcept > 0
                        && this.getDeviance(motion) <= this.teleportExcept) {
                    this.setPredict(motion);
                }
                this.teleportTicks--;
                 ...
                 */
                if (!this.protocol.isUsingVersionOrGreater(MultiVersion.MCVersion.V1_19)
                        && (air > 5 && air < 10)
                    ...
                    if (this.getDeviance(motion) < allowedDeviantity)
                        this.setPredict(motion);
                }
                 ...
                 */
                if (this.protocol.getComponentY().explosionTick && this.getDeviance(motion) < 1.2) {
                    this.setPredict(motion);
                    this.protocol.getComponentY().explosionTick = false;
                }
                if (this.allowMinorGravityChange && this.predictY() > motion
                        && getDeviance(motion) <= 0.08) {
                    this.setPredict(motion);
                    this.allowMinorGravityChange = false;
                }
                if (this.allowLowMinorAccel && this.predictY() > motion && getDeviance(motion) <= 0.1) {
                    this.setPredict(motion);
                    this.allowLowMinorAccel = false;
                }
                if (this.protocol.predictedSlimeTicks > 0 && getDeviance(motion) < 2.5 && getDeviance(motion) > 0.01) {
                    this.setPredict(motion);
                ...
                if (devTotal > ((this.protocol.getVehicle()
                    ...
                    if (devTotal < 1.9 && this.vhact > 0) {
                        ...
                        if (devTotal < 0.1) { // vagdedes patch
                            for (PotionEffectType type : new PotionEffectType[]{
                                    PlayerUtils.levitation ? PotionEffectType.LEVITATION : null,
                                    PlayerUtils.slowFall ? PotionEffectType.SLOW_FALLING : null,
                                    PotionEffectUtils.JUMP
                            }) {
                                if (type != null
                                        && p.hasPotionEffect(type, 5)) {
                                    return;
                                }
                            }
                        }
                            ...
                            if (this.vlCount > ((this.air == 1) ? 0 : (this.vl > 0.3) ? 0 : (this.vl > 0.16) ? 1 : 3)) {
                                double difference = Math.max(
                                        this.hackType.getCheck().getDecimalOption(
                                                "minimum_gravity_difference",
                                                0.0
                                        ),
                                        0.0
                                );

                                if (devTotal > difference) {
                                    this.detection.cancel(
                                            1.0 + devTotal,
                                            "simulation[gravity], y: " + alertBuilder + ", ry: " + devTotal,
    ...
    private double getDeviance(double motion) {
    ...
    void teleport() {
        this.detection.call(() -> {
            ...
            this.teleportTicks = CheckDetection.getLastPrevention() <= (TPS.tickTime * 2L) ? 1 : 5;
            this.vl /= 1.5;
            this.setPredict(0.0);
        });
    }
    ...
    void groundSp(boolean isOnGround) {
        this.detection.call(() -> this.ground(isOnGround));
    }
