...
public class FastBreak extends CheckRunner {
                    ...
                    if (time > 0L) {
                        ...
                        if (time > 0L) { // Make sure the plugin knows when the player started breaking the block
                            ...
                            if (limit > -1L) { // Make sure the limit can be calculated on this specific server
                                ...
                                // stupid false positive fix
                                if (block.getY() <= this.protocol.getLocation().getBlockY()
                                        && Math.abs(block.getX() - this.protocol.getLocation().getBlockX()) <= 1.5
                                        && Math.abs(block.getZ() - this.protocol.getLocation().getBlockZ()) <= 1.5) {
                                    limit /= 2;
                                }
    ...
    private void checkBreaksPerSecond(int seconds, Block block) {
        this.breaksPerSecond.call(() -> {
            ...
            if (gameMode == GameMode.CREATIVE) {
                ...
                if (limit > 0) {
                    double buffer = breaksPerSecondBuffer.count(1, 20);
    ...
    protected boolean canRun() {
        return !MineBomb.isUsing(this.protocol)
                ...
                && !Compatibility.CompatibilityType.ADVANCED_ENCHANTMENTS.isFunctional()
                && Attributes.getAmount(this.protocol, Attributes.PLAYER_BLOCK_BREAK_SPEED) == Double.MIN_VALUE;
    }
